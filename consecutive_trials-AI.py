...
possible_events_s1 = ['in_base', 'reward_time_started', 'not_in_base',     'rat_left_base_during_reward_time', 'correct_trial', 'correct_or_late',    'left_early', 'already_in_base', 'missed_trial', 'error']
...
def graph_normalized_overtime(shape, train, pretrain):
    ...
    plt.title('Probabilities of all rats from shaping ' + str(shape) + ' for training ' + str(train) + ' with pretraining ' + str(pretrain))
    ...
    for i in range(len(wanted)):
        for j in range(size):
            ...
            plt.xticks(rotation=90)
        plt.title('Conditional probabilities given ' + labels[wanted[i]] + ' trial with stddev of shaping ' + str(shape)) 
        plt.legend(loc='upper left')
        plt.show()
...
def graph_segmented_days(data, shape): #data is dict with timestamps as keys and array of [m, e, l, c] as values for that day/time
    '''
    ...
    fig, ax = plt.subplots()
    ax.stackplot(times, segmented[5], segmented[4], segmented[3], segmented[1], segmented[2], segmented[0], labels=labels)
    ax.legend(loc='upper left')
    plt.xticks(rotation=90)
    plt.show()
...
def graph_segmented_perday(path, size):
    ...
    fig, ax = plt.subplots()
    labels = ["correct", "distracted ok", "distracted", "early", "late", "missed"]
    ax.stackplot(mins, totalpermin[5], totalpermin[4], totalpermin[3], totalpermin[1], totalpermin[2], totalpermin[0], labels=labels)
    ax.legend(loc='upper left')
    plt.xticks(rotation=90)
    plt.show()
